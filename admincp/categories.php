...
<div class="page-header page-header-light">
	<div class="page-header-content header-elements-md-inline">
		<div class="page-title d-flex">
			<h4><i class="icon-list-unordered mr-2"></i> <span class="font-weight-semibold">Main Categories</span></h4>
			<a href="#" class="header-elements-toggle text-default d-md-none"><i class="icon-more"></i></a>
		</div>

		<div class="header-elements d-none">
			<div class="d-flex justify-content-center">
				<button id="add-new" class="btn btn-link btn-float text-default"><i class="icon-plus3 text-primary"></i><span>Add Category</span></button>
			</div>
		</div>
	</div>
...
<div class="content">
	...
	<div class="card">
		...
		<div class="card-body">
			Total of <?php $categories = mysql_result(mysql_query("SELECT count(*) FROM `categories` WHERE `parent`=0"),0); _e( appendzero($categories) ); ?> categories found!
		</div>

		<?php if( $categories > 0 ){
			$category_top_rank = category_top_rank(0);
			$category_bottom_rank = category_bottom_rank(0);
			$cat_query = mysql_query("SELECT * FROM `categories` WHERE `parent`=0 ORDER BY `rank` ASC");
		?>
